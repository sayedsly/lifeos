        ...
        <div style={card}>
          ...
          {!agentDone && (
            <div style={{ marginTop: "12px", display: "flex", gap: "8px" }}>
              <input value={followUpText} onChange={e => setFollowUpText(e.target.value)}
                onKeyDown={e => { if (e.key === "Enter" && followUpText.trim()) { const t = followUpText.trim(); setFollowUpText(""); setIsFollowUp(true); resetForFollowUp(); setTimeout(() => { setAgentResult(null); submitText(t); }, 50); } }}
              ...
              <button onClick={() => { if (followUpText.trim()) { const t = followUpText.trim(); setFollowUpText(""); setIsFollowUp(true); resetForFollowUp(); setTimeout(() => { setAgentResult(null); submitText(t); }, 50); } else { handleAgentFollowUp(); } }}
