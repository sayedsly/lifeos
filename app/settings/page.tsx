...
export default function SettingsPage() {
  ...
  if (!s) return <div style={{ padding: "32px", color: "#9ca3af", fontSize: "12px" }}>Loading...</div>;

  const field = (label: string, value: number | string, onChange: (v: string) => void, type = "number") => (
    <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
      <p style={{ fontSize: "9px", fontWeight: 600, letterSpacing: "0.2em", color: "#9ca3af", textTransform: "uppercase" }}>{label}</p>
      <input type={type} value={value} onChange={e => onChange(e.target.value)}
        style={{ width: "100%", background: "#f1f5f9", border: "none", borderRadius: "12px", padding: "12px 16px", color: "#111118", fontSize: "16px", fontWeight: 600, outline: "none", boxSizing: "border-box" as const }} />
    </div>
  );

  const section = (title: string, children: React.ReactNode) => (
    <div style={{ background: "white", border: "none", borderRadius: "24px", padding: "20px", display: "flex", flexDirection: "column", gap: "16px" }}>
      <p style={{ fontSize: "9px", fontWeight: 600, letterSpacing: "0.2em", color: "#9ca3af", textTransform: "uppercase" }}>{title}</p>
      {children}
    </div>
  );
  ...
  const toggle = (label: string, value: boolean, onChange: (v: boolean) => void) => (
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
      <p style={{ color: "#111118", fontSize: "14px", fontWeight: 500 }}>{label}</p>
      <button onClick={() => onChange(!value)} style={{
        ...
        <div style={{ width: "20px", height: "20px", borderRadius: "50%", background: value ? "black" : "#9ca3af", position: "absolute", top: "3px", left: value ? "21px" : "3px", transition: "left 200ms" }} />
      </button>
    </div>
  );
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
      <div style={{ paddingTop: "8px" }}>
        <p style={{ fontSize: "10px", fontWeight: 600, letterSpacing: "0.2em", color: "#9ca3af", textTransform: "uppercase" }}>More</p>
        <p style={{ fontSize: "20px", fontWeight: 700, color: "#111118", marginTop: "4px" }}>Settings</p>
      </div>

      <div style={{ background: "white", border: "none", borderRadius: "24px", overflow: "hidden" }}>
        {[{ href: "/workout", label: "💪 Workout" }, { href: "/finance", label: "💰 Finance & Goals" }, { href: "/nutrition", label: "🥗 Nutrition" }, { href: "/tasks", label: "✅ Tasks" }, { href: "/friends", label: "👥 Friends" }, { href: "/achievements", label: "🏆 Achievements" }].map(({ href, label }, i, arr) => (
          <Link key={href} href={href} style={{ display: "block", padding: "16px 20px", color: "#9ca3af", fontSize: "13px", fontWeight: 500, textDecoration: "none", borderBottom: i < arr.length - 1 ? "1px solid #27272a" : "none" }}>
            {label} →
          </Link>
        ))}
      </div>
      ...
      {section("Momentum Weights", <>
        ...
        {Object.entries(weights).map(([key, val]) => (
          <div key={key} style={{ display: "flex", alignItems: "center", gap: "12px" }}>
            <p style={{ fontSize: "12px", color: "#111118", fontWeight: 600, width: "72px", textTransform: "capitalize" }}>{key}</p>
            <input type="range" min={0} max={50} value={val}
              onChange={e => setWeights(prev => ({ ...prev, [key]: parseInt(e.target.value) }))}
              style={{ flex: 1 }} />
            <p style={{ fontSize: "13px", fontWeight: 700, color: "#111118", width: "32px", textAlign: "right" }}>{val}</p>
          </div>
        ))}
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", paddingTop: "4px", borderTop: "1px solid #27272a" }}>
          <p style={{ fontSize: "11px", color: "#9ca3af", textTransform: "uppercase", letterSpacing: "0.1em" }}>Total</p>
              ...
              {ALL_MODULES.map(({ key, label }) => {
                ...
                return (
                  <button key={key} onClick={() => toggleNav(key)} disabled={disabled}
                    ...
                    {selected && <p style={{ color: "#9ca3af", fontSize: "11px", letterSpacing: "0.1em", textTransform: "uppercase" }}>#{navConfig.indexOf(key) + 1}</p>}
                  </button>
                );
              })}
            </div>
      ...
      {supported && section("Notifications — iOS PWA only",
        <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div>
              <p style={{ color: "#111118", fontSize: "14px", fontWeight: 600 }}>Daily Reminders</p>
              <p style={{ color: "#9ca3af", fontSize: "11px", marginTop: "2px" }}>{subscribed ? "Enabled" : "Get reminded to log your data"}</p>
            </div>
          ...
          {subscribed && (
            <button onClick={sendTestNotification}
              style={{ width: "100%", padding: "12px", borderRadius: "12px", background: "white", border: "none", color: "#111118", fontWeight: 600, fontSize: "13px", cursor: "pointer" }}>
      ...
      <button onClick={save} disabled={totalWeight !== 100} style={{
        ...
        color: saved ? "white" : totalWeight !== 100 ? "#9ca3af" : "black",
